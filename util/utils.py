...
def save_logging1(args, test_loss, test_acc, epoch=None, net=None, train_loss=0.0, train_acc=0.0, save_name='last'):
    if args.mode == 'train':
        save_dict_to_json(
            {
                'epoch': epoch,
        ...
        save_dict_to_json(
            ...
            }, os.path.join('visualization_result', args.name, args.netvit, 'metric.json')
        )


def save_logging(args, test_loss, test_acc, epoch=None, net=None,  TrainSetting=None, train_loss=0.0, train_acc=0.0, save_name='last'):
    if args.mode == 'train':
        save_dict_to_json(
            {
                'epoch': epoch,
            ...
            }, os.path.join('checkpoints', args.name, args.netvit, '{}.json'.format(save_name))
        )
        torch.save(net.state_dict(), os.path.join('checkpoints', args.name, args.netvit, '{}.pth'.format(save_name)))
        torch.save((TrainSetting[0],TrainSetting[1].state_dict(),TrainSetting[2].state_dict()), os.path.join('checkpoints', args.name, args.netvit, '{}_TrainSetting.pth'.format(save_name)))
    else:
        save_dict_to_json(
            {
                'Metric': {
...
def segmentation_loss(pred, gt, mesh_path, weight, device, loss_rate=1.8, bandwidth=1.0):
    class_weight = weight[gt].to(device)
    ...
    loss = criterion(pred[gt], gt[gt], class_weight)# + loss_rate * loss_neighbor

    return loss


def vertex_label(mesh_path, pred_face_label, save_path_name, max_label, GT=False):
    mesh = trimesh.load(mesh_path, process=False)
    mesh: trimesh.Trimesh

    if len(pred_face_label.shape)==2:
        pred_face_label=torch.argmax(pred_face_label, dim=1)
    pred_face_label = pred_face_label.numpy()

    colors = np.array([[255, 0, 0], [0, 255, 0], [0, 0, 255], [255, 255, 0], [255, 0, 255], [0, 255, 255]])  # Red, Green, Blue
    colors = colors[pred_face_label]
    ...
    mesh.export(os.path.join('visualization_result', save_path_name, 'faces',
                             ("GT_" if GT else 'face_')+ os.path.splitext(mesh_path.split('/')[-1])[0] + '.ply'))
